...
pub fn run<S, N, P>(init_state: S) -> anyhow::Result<()>
where
    P: DeserializeOwned,
    N: Node<S, P>,
{
    ...
    let mut node: N = Node::from_init(init_state, init).context("node initializing failed")?;
    ...
    for line in stdin {
        let line = line.context("Could not read STDIN from maelstrom")?;
